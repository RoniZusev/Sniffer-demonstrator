...
class User:
    ...
    def create_main_window(self):
        ...
        start_btn = ttk.Button(left_card, text="▶ Start Sniffer (Ctrl+S)", style="Big.TButton",
                              command=lambda: user.start_sniffer(root))
        ...
        analyze_btn = ttk.Button(right_card, text="⚠ Analyze Malware (Ctrl+A)", style="Big.TButton",
                                command=lambda: user.anylaze_problems(root))
        analyze_btn.grid(row=2, column=0, sticky="w")

        # Keyboard shortcuts
        root.bind_all("<Control-s>", lambda _e: user.start_sniffer(root))
        root.bind_all("<Control-S>", lambda _e: user.start_sniffer(root))
        root.bind_all("<Control-a>", lambda _e: user.anylaze_problems(root))
        root.bind_all("<Control-A>", lambda _e: user.anylaze_problems(root))
    ...
    def anylaze_problems(self, parent):
        ...
        tk.Label(summary_frame, text=f"Total captured packets: {total}", fg="white", bg="#1e1e1e",
                 font=("Segoe UI", 11)).pack(side="left", padx=5)
        tk.Label(summary_frame, text=f"Most common protocol: {most_proto} ({most_proto_count})", fg="white",
                 bg="#1e1e1e",
